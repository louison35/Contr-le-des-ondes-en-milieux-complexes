...
def S_matrix_opt(alpha,G1, Gdd1,G_t1, k1=k):
...
def S_matrix(x1, y1, alpha, k1=k):
    G1 = torch.tensor(green_in(x1, y1, k), dtype=torch.cfloat)
    Gdd1 = green_dd(x1, y1, k)
...
def coef(M):
    ...
    for i in range(N):
        t_moy += t[i,i]
        r_moy += r[i,i]
    return (t_moy/N),(r_moy/N),1-(r_moy/N)

def matrix(par):
    par = torch.cat((par,par_diff))
    par1 = 1/-(-par)
    alpha = torch.diag(par1)
    S = S_matrix_opt(alpha,G,Gdd,G_t)-S0
    coste = torch.sum((abs(S))**2)
    return coste
...
params = torch.tensor([0.01+0.01*torch.rand(1).item()*1j for _ in range(n)], requires_grad=True)
par_diff = torch.tensor([6j for i in range(n_diff)])
...
G = torch.tensor(green_in(x_tot, y_tot, k), dtype=torch.cfloat)
G_t = torch.transpose(G, 0, 1)
Gdd = green_dd(x_tot, y_tot, k)
...
for iteration in range(num_iterations):
    ...
    if iteration % 100 == 0:
        print(f"Iteration {iteration}: <A> ={(cost).item():.4f}")

print(f"Final parameters: {params}")


S = (S_matrix_opt(matrice(params),G,Gdd, G_t)-S0).detach().numpy()
S_test = np.dot(np.conj(np.transpose(S)),S)
t = S[N:,:N]
t1 = np.dot(np.conj(np.transpose(t)),t)

plt.rcParams['figure.dpi'] = 300
plt.imshow(abs(S),vmin = 0,vmax = 1)
plt.colorbar()
plt.show()
