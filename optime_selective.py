...
def S_matrix_opt(alpha,G1, Gdd1,G_t1, k1,N1):
...
params = torch.tensor([0.0001 for _ in range(len(x))], requires_grad=True)
...
f = 7 * 10**9
nf = 2
ft = np.linspace(f-0.05*10**9,f+0.05*10**9,nf)
...
for iteration in range(num_iterations):
    ...
    if iteration % 100 == 0:
        print(f"Iteration {iteration}: <R> ={(cost).item()/nf:.4f}")
